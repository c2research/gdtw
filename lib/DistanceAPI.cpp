        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    genex::data_t *inMatrix_1;     /* 1xN input matrix */
    genex::data_t *inMatrix_2;     /* 1xN input matrix */
    genex::data_t *outMatrix;      /* output matrix    */
        
    /* create a pointer to the real data in the first input matrix  */
    inMatrix_1 = mxGetPr(prhs[0]);
    const genex::TimeSeries x_1(mxGetPr(prhs[0]), mxGetN(prhs[0]));

    /* create a pointer to the real data in the second input matrix  */
    inMatrix_2 = mxGetPr(prhs[1]);
    const genex::TimeSeries x_2(mxGetPr(prhs[1]), mxGetN(prhs[1]));
    ...
    /* get distance */
    const genex::dist_t distance = genex::getDistance(in_name + "_warp");
